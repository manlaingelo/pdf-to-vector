...
def perform_clustering(collection: chromadb.Collection):
    ...
    results = collection.get(
        ids=collection.get()["ids"], include=["embeddings"]  # Get all IDs
    )
    ...
    if len(vectors) < 2:
        print(
            f"🚫 Not enough data ({len(vectors)} points) to perform clustering. Need at least 2 points."
        )
        return

    if k < K_CLUSTERS:
        print(
            f"⚠️  Adjusting K from {K_CLUSTERS} to {k} due to limited data ({len(vectors)} points)."
        )
    ...
    for doc_id, label in tqdm(
        zip(ids_to_update, cluster_labels), total=len(ids_to_update), desc="Updating DB"
    ):
