...
RELEVANCE_THRESHOLD = (
    0.3  # Minimum relevance score (0-1) to consider documents relevant
)
...
class OllamaEmbeddingFunction(embedding_functions.EmbeddingFunction):
    """Custom embedding function for ChromaDB using local Ollama."""
...
def retrieve_relevant_docs(
    collection: chromadb.Collection, query: str, n_results: int = TOP_K_RESULTS
):
...
ANSWER:"""
    ...
    try:
        ...
        if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
            ...
            return (
                "⚠️ API quota exceeded. Please check your Google AI Studio quota limits."
            )
        else:
            raise
    ...
    while True:
        ...
        try:
            ...
            if not results["documents"][0]:
                ...
                print(
                    "💡 This question appears to be outside the scope of the loaded documents.\n"
                )
                continue
            ...
            for i, metadata in enumerate(results["metadatas"][0][:3]):
                print(
                    f"  {i+1}. {metadata.get('source')} (Page {metadata.get('page')})"
                )
            print()
